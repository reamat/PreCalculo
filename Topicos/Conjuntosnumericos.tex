...
\begin{prop}[Propriedades do módulo]
 Para $a$ e $b$ números reais quaisquer, são válidas as seguintes propriedades: \label{prop.modulo}
\begin{enumerate}
 \item $\abs{a} \geq 0$;
 \item $\abs{a}= 0 \Leftrightarrow a= 0$;
 \item $\abs{a} \geq a$;
 \item $\abs{-a}= \abs{a}$;
 \item $\abs{a \cdot b}= \abs{a} \cdot \abs{b}$;
 \item $\abs{\frac{a}{b}}= \frac{\abs{a}}{\abs{b}}$;
 \item $\abs{a^n}= \abs{a}^n$, se $n$ é par;
 \item $\abs{a+b}\leq \abs{a}+\abs{b}$, desigualdade triangular;
 \item $\abs{a - b} \geq \abs{a} - \abs{b}$;
 \item $\abs{\abs{a} - \abs{b}} \leq \abs{a} - \abs{b}$;
